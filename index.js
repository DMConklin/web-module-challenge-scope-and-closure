...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
      The count variable in counter1 is private while counter for counter 2 is globalally accessible, which means count for counter2 can be changed by means other than the counter2 function
  2. Which of the two uses a closure? How can you tell?
      counter1
  3. In what scenario would the counter1 code be preferable? In what scenario would 
     counter2 be better?  
     counter1 would be preferrable when you wanted data privacy and counter2 would be preferrable if you had other functions that needed to change the count variable
*/
...
function scoreboard(inningScore, score, innings/* CODE HERE */) {
  /* CODE HERE */
  let totalScore = {
    Away: 0,
    Home: 0
  ...
  for (let i = 1; i <= innings; i++) {
    let scoreObj = inningScore(score)
    scoreArray.push(`Inning ${i}: Away ${scoreObj.Away} - Home ${scoreObj.Home}`)
    totalScore.Home += scoreObj.Home
    totalScore.Away += scoreObj.Away
  }
  if (totalScore.Home === totalScore.Away) {
    scoreArray.push(`This game will require extra innings: Away ${totalScore.Away} - Home ${totalScore.Home}`)
  } else {
    scoreArray.push(`Final Score: Away ${totalScore.Away} - Home ${totalScore.Home}`)
  }
  return scoreArray
}
